document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderQuickSearchCard(statusHtml, options = {}) {
    currentRenderedMovieId = null;

    // Keep the input and its results while the active-tab check changes state.
    // Replacing this card used to eject focus and make non-film pages appear
    // unable to search.
    ...
    const iconHtml = icon ? `<div class="empty-icon" style="font-size: 28px;">${icon}</div>` : "";
    const style = options.borderColor ? ` style="border-left: 3px solid ${options.borderColor};"` : "";
    playingContainer.innerHTML = `
      <div class="card empty-playing quick-search-empty"${style}>
        ${iconHtml}
        <div id="quickSearchStatus" class="quick-search-status">${statusHtml}</div>
  ...
  function renderNonMovieSkipped(htmlMsg) {
    ...
    `, { icon: "📺", borderColor: "#38bdf8" });
  }

  function renderEmptyPlaying(msg) {
    renderQuickSearchCard(msg);
  }
